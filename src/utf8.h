...
utf8_constexpr14 utf8_nonnull utf8_pure size_t utf8size_lazy(
...
utf8_constexpr14 utf8_nonnull utf8_pure size_t utf8nsize_lazy(
...
utf8_constexpr14 utf8_nonnull size_t utf8codepointcalcsize(
    const utf8_int8_t *str);
...
utf8casecmp(const utf8_int8_t *src1, const utf8_int8_t *src2)
{
	...
	for (;;) {
		...
		/* if they don't match, then we return the difference between
		 * the characters
		 */
...
utf8_constexpr14_impl int
utf8cmp(const utf8_int8_t *src1, const utf8_int8_t *src2)
{
	while (('\0' != *src1) || ('\0' != *src2)) {
		if (*src1 < *src2) {
			return -1;
...
utf8cspn(const utf8_int8_t *src, const utf8_int8_t *reject)
{
	...
	while ('\0' != *src) {
		...
		while ('\0' != *r) {
			 ...
			 * (it is not 0b10xxxxxx) and we have successfully
			 * matched a previous character (0 < offset) - we found
			 * a match */
			if ((0x80 != (0xc0 & *r)) && (0 < offset)) {
				return chars;
			} else {
				if (*r == src[offset]) {
					/* part of a utf8 codepoint matched, so
					 * move our checking onwards to the next
					 * byte */
					offset++;
					r++;
				} else {
					/* r could be in the middle of an
					 * unmatching utf8 code point, so we
					 * need to march it on to the next
					 * character beginning, */
					...
					/* reset offset too as we found a
					 * mismatch */
					offset = 0;
				}
			}
		...
		/* found a match at the end of *r, so didn't get a chance to
		 * test it */
		if (0 < offset) {
			return chars;
		}

		/* the current utf8 codepoint in src did not match reject, but
		 * src could have been partway through a utf8 codepoint, so we
		 * need to march it onto the next utf8 codepoint starting byte
		 */
		do {
			src++;
		} while ((0x80 == (0xc0 & *src)));
...
utf8_int8_t *
utf8dup(const utf8_int8_t *src)
{
	return utf8dup_ex(src, utf8_null, utf8_null);
}

utf8_int8_t *
utf8dup_ex(const utf8_int8_t *src,
    utf8_int8_t *(*alloc_func_ptr)(utf8_int8_t *, size_t),
    utf8_int8_t *user_data)
{
...
utf8_constexpr14_impl size_t
utf8len(const utf8_int8_t *str)
{
	return utf8nlen(str, SIZE_MAX);
}

utf8_constexpr14_impl size_t
utf8nlen(const utf8_int8_t *str, size_t n)
{
...
utf8ncasecmp(const utf8_int8_t *src1, const utf8_int8_t *src2, size_t n)
{
	...
	do {
		...
		/* first check that we have enough bytes left in n to contain an
		 * entire codepoint */
		if (0 == n) {
			return 0;
		}
...
utf8_int8_t *
utf8ndup(const utf8_int8_t *src, size_t n)
{
	return utf8ndup_ex(src, n, utf8_null, utf8_null);
}

utf8_int8_t *
utf8ndup_ex(const utf8_int8_t *src, size_t n,
    utf8_int8_t *(*alloc_func_ptr)(utf8_int8_t *, size_t),
    utf8_int8_t *user_data)
{
...
utf8pbrk(const utf8_int8_t *str, const utf8_int8_t *accept)
{
	while ('\0' != *str) {
		...
		/* the current utf8 codepoint in src did not match accept, but
		 * src could have been partway through a utf8 codepoint, so we
		 * need to march it onto the next utf8 codepoint starting byte
		 */
		do {
...
utf8_constexpr14_impl size_t
utf8size(const utf8_int8_t *str)
{
...
utf8spn(const utf8_int8_t *src, const utf8_int8_t *accept)
{
	...
	while ('\0' != *src) {
		...
		while ('\0' != *a) {
			 ...
			 * a match */
			if ((0x80 != (0xc0 & *a)) && (0 < offset)) {
				/* found a match, so increment the number of
				 * utf8 codepoints that have matched and stop
				 * checking whether any other utf8 codepoints in
				 * a match */
				chars++;
				src += offset;
				offset = 0;
				...
				if (*a == src[offset]) {
					...
					/* a could be in the middle of an
					 * unmatching utf8 codepoint, so we need
					 * to march it on to the next character
					 * beginning, */
					do {
						a++;
					} while (0x80 == (0xc0 & *a));
		...
		/* found a match at the end of *a, so didn't get a chance to
		 * test it */
		if (0 < offset) {
			chars++;
			src += offset;
		...
		/* if a got to its terminating null byte, then we didn't find a
		 * match. Return the current number of matched utf8 codepoints
		 */
		if ('\0' == *a) {
			return chars;
		}
...
utf8str(const utf8_int8_t *haystack, const utf8_int8_t *needle)
{
	...
	while ('\0' != *haystack) {
		...
		if ('\0' == *n) {
			/* we found the whole utf8 string for needle in haystack
			 * at maybeMatch, so return it */
			return (utf8_int8_t *)maybeMatch;
		} else {
			/* h could be in the middle of an unmatching utf8
			 * codepoint, so we need to march it on to the next
			 * character beginning starting from the current
			 * character */
			haystack =
			    utf8codepoint(maybeMatch, &throwaway_codepoint);
		}
	}
...
utf8casestr(const utf8_int8_t *haystack, const utf8_int8_t *needle)
{
	...
	for (;;) {
		...
		const utf8_int8_t *nextH = haystack =
		    utf8codepoint(haystack, &h_cp);
...
utf8nvalid(const utf8_int8_t *str, size_t n)
{
	...
	while ((void)(consumed = (size_t)(str - t)),
	    consumed < n && '\0' != *str) {
...
int
utf8makevalid(utf8_int8_t *str, const utf8_int32_t replacement)
{
    ...
    utf8_int32_t *utf8_restrict out_codepoint)
{
	if (0xf0 == (0xf8 & str[0])) {
		...
		*out_codepoint = ((0x07 & str[0]) << 18) |
				 ((0x3f & str[1]) << 12) |
				 ((0x3f & str[2]) << 6) | (0x3f & str[3]);
		str += 4;
	} else if (0xe0 == (0xf0 & str[0])) {
		/* 3 byte utf8 codepoint */
		*out_codepoint = ((0x0f & str[0]) << 12) |
				 ((0x3f & str[1]) << 6) | (0x3f & str[2]);
		str += 3;
...
utf8_constexpr14_impl size_t
utf8codepointsize(utf8_int32_t chr)
{
...
utf8catcodepoint(utf8_int8_t *str, utf8_int32_t chr, size_t n)
{
	if (0 == ((utf8_int32_t)0xffffff80 & chr)) {
		...
		str[0] =
		    (utf8_int8_t)(0xe0 | (utf8_int8_t)((chr >> 12) & 0x0f));
		...
		str[0] =
		    (utf8_int8_t)(0xf0 | (utf8_int8_t)((chr >> 18) & 0x07));
		str[1] =
		    (utf8_int8_t)(0x80 | (utf8_int8_t)((chr >> 12) & 0x3f));
...
void
utf8lwr(utf8_int8_t *utf8_restrict str)
{
	utf8_int32_t cp = 0;
...
void
utf8upr(utf8_int8_t *utf8_restrict str)
{
	utf8_int32_t cp = 0;
	utf8_int8_t *pn = utf8codepoint(str, &cp);
    ...
    utf8_int32_t *utf8_restrict out_codepoint)
{
	...
	if (0xf0 == (0xf8 & s[0])) {
		...
		*out_codepoint = ((0x07 & s[0]) << 18) | ((0x3f & s[1]) << 12) |
		...
		/* 3 byte utf8 codepoint */
		*out_codepoint = ((0x0f & s[0]) << 12) | ((0x3f & s[1]) << 6) |
				 (0x3f & s[2]);
