...
void read_pack(object *pack, FILE *fp, bool is_rogue)
{
 ...
 */
void rw_dungeon(FILE *fp, bool rw)
{
    short i, j;
...
void rw_id(struct id id_table[], FILE *fp, int n, bool wr)
{
    short i;
...
void rw_rooms(FILE *fp, bool rw)
{
    short i;

    for (i = 0; i < MAXROOMS; i++)
    {
        rw ? r_write(fp, (char *)(rooms + i), sizeof(room)) : r_read(fp, (char *)(rooms + i), sizeof(room));
    }
}
...
bool has_been_touched(struct rogue_time *saved_time, struct rogue_time *mod_time)
{
    short *sav, *mod;
    int i;
