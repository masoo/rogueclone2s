...
void
mvaddstr(short row, short col, char *str)
{
    move(row, col);
    addstr(str);
}
...
put_cursor(register int row, register int col)
{
    ...
    *p = 0;
	...
	if (cdif <= 6) {
	    for (i = 0; i < cdif; i++) {
		ch = (col < cur_col) ? BS : terminal[row][cur_col + i];
		put_st_char((int) ch);
	    }
	    cur_row = row;
	    cur_col = col;
...
/* rewritten by Yasha */
	...
	if (strlen(tcf) > 40 || (fp = fopen(tcf, "r")) == NULL) {
    ...
    } else if (!(tc_file = tcf ? tcf : md_gdtcf()))
...
/*tc_gtdata(fp, buf)*/
void
tc_gtdata(char *tcf, FILE * fp, char *buf)
{				/* by Yasha */
    /* tcf: termcap (by Yasha) */
    short   i;
    boolean first = 1;
