 ...
 * 取得した環境変数の値を変数に格納する
 */
void env_get_value(char **s, char *e, bool add_blank, bool no_colon)
{
    int i = 0;
    char *t;
    ...
    while ((*e) && (*e != ','))
    {
        ...
        if (*e & 0x80)
        {
            if ((*e >= '\xA1' && *e <= '\xFE') && (*(e + 1) >= '\xA1' && *(e + 1) <= '\xFE'))
            {
                /* 漢字 */
                e += 2;
                i += 2;
            }
            else if ((*e == '\x8E') && ((*(e + 1) >= '\xA0') && (*(e + 1) <= '\xDF')))
            {
                /* 半角カナ */
                e += 2;
                i += 2;
            }
            else if ((*e == '\x8F') && ((*(e + 1) >= '\xA1') && (*(e + 1) <= '\xFE')) && ((*(e + 2) >= '\xA1') && (*(e + 2) <= '\xFE')))
            {
                /* 補助漢字 */
                e += 3;
                i += 3;
