...
gr_row_col(short *row, short *col, unsigned short mask)
{
	...
	} while ((rn == NO_ROOM) || (!(dungeon[r][c] & mask)) ||
		 (dungeon[r][c] & (~mask)) ||
...
{
	short i,
	    starting_room = 0; /* 未初期化変数の警告除去のため 0 で初期化 */
...
draw_magic_map(void)
{
	short i, j, ch, och;
	unsigned short mask =
	    (HORWALL | VERTWALL | DOOR | TUNNEL | TRAP | STAIRS | MONSTER);
	unsigned short s;
...
dr_course(object *monster, bool entering, short row, short col)
{
	...
	if (entering) { /* entering room */
		...
		for (i = 0; i < MAXROOMS; i++) {
			rr = (r + i) % MAXROOMS;
			if ((!(rooms[rr].is_room & (R_ROOM | R_MAZE))) ||
			    (rr == rn)) {
				continue;
			}
			for (k = 0; k < 4; k++) {
				if (rooms[rr].doors[k].oth_room == rn) {
					monster->trow =
					    rooms[rr].doors[k].oth_row;
					monster->tcol =
					    rooms[rr].doors[k].oth_col;
					if ((monster->trow == row) &&
					    (monster->tcol == col)) {
						continue;
					}
