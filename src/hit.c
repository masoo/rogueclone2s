...
mon_hit(object *monster, char *other, bool flame)
{
	...
	if (!(monster->m_flags & STATIONARY)) {
		...
		if (other) {
			if (flame) {
				if ((damage -= get_armor_class(rogue.armor)) <
				    0) {
					damage = 1;
				}
			}
...
fight(bool to_the_death)
{
	...
	while (fight_monster) {
		...
		if (((!to_the_death) &&
			(rogue.hp_current <= possible_damage)) ||
...
get_dir_rc(char dir, short *lines, short *columns, bool allow_off_screen)
{
	...
	case 'u':
		if (allow_off_screen ||
		    ((*lines > MIN_ROW) && (*columns < (ROGUE_COLUMNS - 1)))) {
			(*lines)--;
			(*columns)++;
		}
		break;
