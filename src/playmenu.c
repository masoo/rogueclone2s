...
typedef struct machine { char *mtype, *cursor, *init, *term; int ltype;} mac;
	...
	int by, bx, ey, ex;
{
	...
	color(lcolor);
	move(by, bx); xputs(linec[TOPL]);
	for (n = bx+2; n < ex; n += 2)
		xputs(linec[HOR]);
	xputs(linec[TOPR]);
	for (n = by+1; n < ey; n++) {
		move(n, bx); xputs(linec[VERL]);
		move(n, ex); xputs(linec[VERR]);
	}
	move(ey, bx); xputs(linec[BOTL]);
	for (n = bx+2; n < ex; n += 2)
		xputs(linec[HOR]);
	xputs(linec[BOTR]);
	color(WHITE);
 ...
 */
get()	/* get menu selection character */
{
	register int c;
