...
int connect_rooms(short room1, short room2)
{
    ...
    if (same_row(room1, room2))
    {
        ...
        { /*if (rooms[room2].left_col > rooms[room1].right_col) */
    ...
    else if (same_col(room1, room2))
    {
        ...
        { /*if (rooms[room2].top_row > rooms[room1].bottom_row) */
...
void fill_it(int rn, bool do_rec_de)
{
    int i;
...
bool mask_room(short rn, short *row, short *col, unsigned short mask)
{
    int i, j;
...
void show_average_hp(void)
{
    ...
    {
        real_average = ((rogue.hp_max - extra_hp - INIT_HP) + less_hp) * 100L / (rogue.exp - 1);
        effective_average = (rogue.hp_max - INIT_HP) * 100L / (rogue.exp - 1);
    }
