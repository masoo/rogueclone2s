...
long level_points[MAX_EXP_LEVEL] = {10L, 20L, 40L, 80L, 160L, 320L, 640L, 1300L,
	2600L, 5200L, 10000L, 20000L, 40000L, 80000L, 160000L, 320000L,
	1000000L, 3333333L, 6666666L, MAX_EXP, 99900000L};
		...
		for (i = 0; i < MAXROOMS; i++) {
			...
			if (rooms[j].is_room & R_NOTHING) {
				if (rand_percent(maze_percent)) {
					...
					make_maze(get_rand(rooms[j].top_row + 1,
						      rooms[j].bottom_row - 1),
					    ...
					    rooms[j].left_col,
					    rooms[j].right_col);
					hide_boxed_passage(rooms[j].top_row,
					    rooms[j].left_col,
					    rooms[j].bottom_row,
					    rooms[j].right_col, get_rand(0, 2));
				}
			}
		}
	}
...
fill_it(int rn, bool do_rec_de)
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (((target_room < 0) || (target_room >= MAXROOMS)) ||
		    (!(same_row(rn, target_room) ||
			same_col(rn, target_room))) ||
		    (!(rooms[target_room].is_room & (R_ROOM | R_MAZE)))) {
			continue;
		}
		if (same_row(rn, target_room)) {
			tunnel_dir =
			    (rooms[rn].left_col < rooms[target_room].left_col)
				? RIGHT
				: LEFT;
		} else {
			tunnel_dir =
			    (rooms[rn].top_row < rooms[target_room].top_row)
				? DOWN
				: UPWARD;
		}
		door_dir = ((tunnel_dir + 4) % DIRS);
		if (rooms[target_room].doors[door_dir / 2].oth_room !=
		    NO_ROOM) {
			continue;
		}
...
make_maze(short r, short c, short tr, short br, short lc, short rc)
{
	...
	for (i = 0; i < 4; i++) {
		switch (dirs[i]) {
		case UPWARD:
			if (((r - 1) >= tr) && (dungeon[r - 1][c] != TUNNEL) &&
			...
			break;
		case DOWN:
			if (((r + 1) <= br) && (dungeon[r + 1][c] != TUNNEL) &&
			    (dungeon[r + 1][c - 1] != TUNNEL) &&
			    (dungeon[r + 1][c + 1] != TUNNEL) &&
			    (dungeon[r + 2][c] != TUNNEL)) {
				make_maze((r + 1), c, tr, br, lc, rc);
			...
			break;
		case LEFT:
			if (((c - 1) >= lc) && (dungeon[r][c - 1] != TUNNEL) &&
			    (dungeon[r - 1][c - 1] != TUNNEL) &&
			    (dungeon[r + 1][c - 1] != TUNNEL) &&
			...
			break;
		case RIGHT:
			if (((c + 1) <= rc) && (dungeon[r][c + 1] != TUNNEL) &&
			    (dungeon[r - 1][c + 1] != TUNNEL) &&
			    (dungeon[r + 1][c + 1] != TUNNEL) &&
			    (dungeon[r][c + 2] != TUNNEL)) {
				make_maze(r, (c + 1), tr, br, lc, rc);
...
hide_boxed_passage(short row1, short col1, short row2, short col2, short n)
{
	...
	if (cur_level > 2) {
		...
		if ((w >= 5) || (h >= 5)) {
			...
			for (i = 0; i < n; i++) {
				for (j = 0; j < 10; j++) {
					row = get_rand(row1 + row_cut,
					    row2 - row_cut);
					col = get_rand(col1 + col_cut,
					    col2 - col_cut);
...
{ /* try not to put in this room */
	...
	wake_room(get_room_number(rogue.row, rogue.col), 1, rogue.row,
	    rogue.col);
...
show_average_hp(void)
{
	...
	messagenf(80, 0, mesg[54], (int)(real_average / 100L),
