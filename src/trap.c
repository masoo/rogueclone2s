...
char *trap_strings[TRAPS * 2] = {mesg[216], mesg[217], mesg[218], mesg[219],
	mesg[220], mesg[221], mesg[222], mesg[223], mesg[224], mesg[225],
	mesg[226], mesg[227]};
	...
	for (s = 0; s < n; s++) {
		for (i = -1; i <= 1; i++) {
			for (j = -1; j <= 1; j++) {
				...
				col = rogue.col + j;
				if ((row < MIN_ROW) ||
				    (row >= (ROGUE_LINES - 1)) || (col < 0) ||
				    (col >= ROGUE_COLUMNS)) {
					continue;
				}
				if (dungeon[row][col] & HIDDEN) {
					if (rand_percent(
						17 + (rogue.exp + ring_exp))) {
						dungeon[row][col] &= (~HIDDEN);
						if ((!blind) &&
						    ((row != rogue.row) ||
							(col != rogue.col))) {
